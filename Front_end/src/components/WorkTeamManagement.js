...
const WorkTeamManagement = () => {
    ...
    const fetchTeamMembers = async () => {
      try {
        const response = await fetchWithAuth(
          "http://localhost:8000/api/detailed-team-members/"
        );
        if (response.ok) {
  ...
  useEffect(() => {
    const fetchTeamLeaders = async () => {
      try {
        const response = await fetchWithAuth(
          "http://localhost:8000/api/team-leaders/"
        );
        const data = await response.json();
  ...
  const availableMembers = teamMembers.filter(
    (member) =>
      ...
      !selectedTeam.members.some(
        (teamMember) =>
          teamMember.australian_sailing_number ===
  ...
  const ClosePopup = () => {
    ...
    setSelectedTeams((prevSelectedTeams) =>
      prevSelectedTeams.filter((team) => team.id !== selectedTeam?.id)
    );
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/teams/${selectedTeam.id}/add-member/`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            members: [
              ...selectedTeam.members.map((m) => m.australian_sailing_number),
              selectedMember,
            ],
          }),
        }
  ...
  const handleRemoveSelectedTeams = async () => {
    if (selectedTeams.length === 0) {
      ...
      setModalMessage(
        `Are you sure you want to remove the selected ${selectedMembers.length} member(s)?`
      );
    }
    ...
    try {
      ...
      for (const team of selectedTeams) {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          setModalMessage(
            `Failed to delete team: ${JSON.stringify(errorData)}`
          );
          setModalOpen(true);
  ...
  const handleCreateTeam = async () => {
    ...
    const currentMembers = existingTeam
      ? existingTeam.members.map((member) => member.australian_sailing_number)
      : [];
    ...
    if (existingTeam) {
      try {
        const response = await fetch(
          `http://localhost:8000/api/detailed-teams/${existingTeam.id}/`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          setModalMessage(
            `Failed to update team: ${JSON.stringify(errorData)}`
          );
          setModalOpen(true);
        }
      } catch (error) {
      ...
      try {
        const response = await fetch(
          "http://localhost:8000/api/detailed-teams/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          setModalMessage(
            `Failed to create team: ${JSON.stringify(errorData)}`
          );
          setModalOpen(true);
        }
      } catch (error) {
  ...
  const handleRemoveMember = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/teams/${selectedTeam.id}/remove-members/`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setModalMessage(
          `Failed to remove members: ${JSON.stringify(errorData)}`
        );
        setModalOpen(true);
      }
    } catch (error) {
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const filtered = availableMembers.filter(
                        (member) =>
                          `${member.first_name.toLowerCase()} ${member.last_name.toLowerCase()}`.includes(
                            query.toLowerCase()
                          ) // Convert only for comparison (not the input)
                      );
                    ...
                    onKeyDown={(e) => {
                      if (!memberSelected) {
                        // Check if the member hasn't been selected yet
                        if (
                          e.key === "ArrowDown" &&
                          ...
                          setHighlightedIndex(
                            (prevIndex) =>
                              (prevIndex + 1) % filteredMembers.length
                        ...
                        if (e.key === "Enter" && highlightedIndex >= 0) {
                          const selectedMember =
                            filteredMembers[highlightedIndex]; // Get the currently highlighted member
                          setSearchQuery(
                            `${selectedMember.first_name} ${selectedMember.last_name}`
                          ); // Set the input field with the selected member's name
                          setSelectedMember(
                      ...
                      {filteredMembers.map((member, index) => (
                        <li
                          ...
                          style={{
                            padding: "10px",
                            backgroundColor:
                              highlightedIndex === index ? "#ddd" : "white", // Highlight the current member
                            cursor: "pointer",
                          }}
                          ...
                          onClick={() => {
                            ...
                            ); // Set input to selected member's name
            ...
            <div className={styles.popupContent}>
              ...
              <div className={styles.editTeam}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewTeam({ ...newTeam, TeamLeader: e.target.value })
                  }
                  required
                >
